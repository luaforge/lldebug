...
PRectangle ListBoxImpl::GetDesiredRect() {
    ...
    int maxw = (int)maxStrWidth * aveCharWidth;
    int maxh ;
...
long Platform::SendScintillaPointer(WindowID w,
    ...
    wxScintilla* sci = (wxScintilla*)w;
    return sci->SendMsg(msg, wParam, (sptr_t)lParam);
}
...
wxString sci2wx(const char* str, size_t len)
{
    ...
    unsigned int wclen = UTF16Length(str, (unsigned int)len);
    wxWCharBuffer buffer(wclen+1);

    unsigned int actualLen = UTF16FromUTF8(str, (unsigned int)len, buffer.data(), wclen+1);
...
const wxWX2MBbuf wx2stc(const wxString& str)
{
    ...
    unsigned int wclen   = (unsigned int)str.length();
    unsigned int len     = UTF8Length(wcstr, wclen);
