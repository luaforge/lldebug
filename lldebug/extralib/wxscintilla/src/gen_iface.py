...
#
# Created:      5-Sept-2000
# RCS-ID:       $Id: gen_iface.py,v 1.1 2008-01-09 04:46:22 cielacanth Exp $
...
H_TEMPLATE    = os.path.abspath('./wxscintilla.h.in')
CPP_TEMPLATE  = os.path.abspath('./wxscintilla.cpp.in')
H_DEST        = os.path.abspath('../include/wx/wxscintilla.h')
CPP_DEST      = os.path.abspath('./wxscintilla.cpp')
DOCSTR_DEST   = os.path.abspath('./_stc_gendocs.i')
...
methodOverrideMap = {
    'AddText' : (0,
                 ...
                 '''void %s(const wxString& text) {
                    ...
                    SendMsg(%s, (uptr_t)strlen(buf), (sptr_t)(const char*)buf);''',
                 0),
    ...
    (0,
     ...
     int %s(const wxString& text) {
         wxWX2MBbuf buf = (wxWX2MBbuf)wx2sci(text);
         return SendMsg(%s, (uptr_t)strlen(buf), (sptr_t)(const char*)buf);''',
     0),
    ...
    (0,
     ...
     '''wxString %s(const wxString& key) {
         ...
         wxMemoryBuffer mbuf(len+1);
         char* buf = (char*)mbuf.GetWriteBuf(len+1);
         SendMsg(%s, (uptr_t)(const char*)wx2sci(key), (sptr_t)buf);
         mbuf.UngetWriteBuf(len);
         mbuf.AppendByte(0);
         return sci2wx(buf);''',
    ...
    'SetFocus'  : ('SetSCIFocus', 0, 0, 0),
    'GetFocus'  : ('GetSCIFocus', 0, 0, 0),
    'SetCursor' : ('SetSCICursor', 0, 0, 0),
    'GetCursor' : ('GetSCICursor', 0, 0, 0),
    ...
    for retType, name, number, param1, param2, docs in methods:
        ...
        if not theImp:
            ...
            theImp = theImp + 'SendMsg(%s, %s, %s)' % (number,
                                                       makeArgString(param1, 1),
                                                       makeArgString(param2, 2))
            if retType == 'bool':
                theImp = theImp + ' != 0'
            if retType == 'wxColour':
    ...
    if methodOverrideMap.has_key(name):
        ...
        try:
            ...
            if item[1] != 0:
                theDef = '    ' + (item[1] % name)
            if item[2] != 0:
                theImp = item[2] % ('wxScintilla::'+name, number) + '\n}'
            if item[3] != 0:
                docs = item[3]
        except:
            print "*************", name
...
def makeArgString(param, argNum):
    if not param:
        return '0'
    ...
    if typ == 'string':
        if argNum == 1:
            return '(uptr_t)(const char*)wx2sci(%s)' % name
        else:
            return '(sptr_t)(const char*)wx2sci(%s)' % name
    if typ == 'colour':
...
def parseVal(line, values, docs):
    ...
    # add it to the list
    values.append( ('wxSCI_' + name, val, docs) )
