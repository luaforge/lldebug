...
//	friend class bbl_translater_type;
  ...
  public:
	...
	virtual void flush() {
		for(size_t i = 0, length = untranslated_buffer.length(); i < length; ++i) {
...
//	typedef typename base_type::to_string_type		to_string_type;
  ...
  protected:
	void translate() { }
	void flush() { this->untranslated_buffer = bbl_term::get_empty(); }
  public:
...
//  private:	//	VC のヽ(｀Д´)ノアホォ！
  ...
  protected:
	void translate() {
		this->translated_buffer += this->untranslated_buffer;
//		this->untranslated_buffer.clear();	//	g++ のヽ(｀Д´)ノボケェ！
		this->untranslated_buffer.erase();
	}
	void flush() { }
  public:
static bbl_translater_type create() { return new this_type; }
...
class enbom_engine :public through_engine<target_string> {
  ...
  private:
	enbom_engine() {
		this->translated_buffer = bbl_term::get_bom();
	}
  protected:
	void clear() {
		base_type::clear();
...
class debom_engine :public through_engine<target_string> {
  ...
  protected:
	void translate() {
		...
		while(from_string_type::npos != (p_bom = this->untranslated_buffer.find(bbl_term::get_bom()))) {
			this->untranslated_buffer.erase(p_bom, 1);
		}
		base_type::translate();
	}
  public:
...
//  private:	//	VC のヽ(｀Д´)ノアホォ！
  ...
  protected:
	void translate() {
		engine1.translate(this->untranslated_buffer);
//		this->untranslated_buffer.clear();	//	g++ のヽ(｀Д´)ノボケェ！
		this->untranslated_buffer.erase();
		engine2.translate(engine1.pook_buffer());
		this->translated_buffer += engine2.pook_buffer();
	}
	void flush() {
		engine1.flush();
...
//  private:	//	VC のヽ(｀Д´)ノアホォ！
  ...
  protected:
	void translate() {
		engine.translate(this->untranslated_buffer);
//		this->untranslated_buffer.clear();	//	g++ のヽ(｀Д´)ノボケェ！
		this->untranslated_buffer.erase();
		this->translated_buffer += engine.pook_buffer();
	}
	void flush() {
		engine.flush();
//		this->untranslated_buffer.clear();	//	g++ のヽ(｀Д´)ノボケェ！
...
//  private:	//	VC のヽ(｀Д´)ノアホォ！
  ...
  protected:
	void translate() {
		int new_from_base_encoding = analyze_base_encoding(this->untranslated_buffer);
		if (from_base_encoding != new_from_base_encoding) {
			from_base_encoding = new_from_base_encoding;
			this->engine = this->order(from_base_encoding, to_base_encoding);
		} else {
//			this->engine.clear();
		}
		this->engine.translate(this->untranslated_buffer);
//		this->untranslated_buffer.clear();	//	g++ のヽ(｀Д´)ノボケェ！
		this->untranslated_buffer.erase();
		this->translated_buffer = this->engine.pook_buffer();
	}
  public:
