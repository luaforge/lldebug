...
// Created:     01/02/97
// RCS-ID:      $Id: treelistctrl.cpp,v 1.1 2008-12-29 02:24:46 cielacanth Exp $
...
static const int EXTRA_HEIGHT = 2;
...
void wxTreeListMainWindow::PaintItem (wxTreeListItem *item, wxDC& dc) {
	...
	int off_w = HasFlag(wxTR_COL_LINES) ? 1 : 0;
    ...
    for (int i = 0; i < GetColumnCount(); ++i ) {
        ...
        case wxALIGN_RIGHT:
            ...
            w = col_w - (image_w + text_w + MARGIN + off_w);
			if (i == GetMainColumn()) w -= x - x_colstart;
            x += (w > 0)? w: 0;
            break;
        case wxALIGN_CENTER:
            dc.GetTextExtent(text, &text_w, NULL);
            w = (col_w - (image_w + text_w + MARGIN + off_w))/2;
            x += (w > 0)? w: 0;
            break;
        }
		...
		if (HasFlag(wxTR_COL_LINES)) { // vertical lines between cols
			// if the background colour is white, choose a
			// contrasting color for the lines
			dc.SetPen (
				*((GetBackgroundColour() == *wxWHITE)?wxLIGHT_GREY_PEN : wxWHITE_PEN));
			int line_x = x_colstart + col_w - 1;
			dc.DrawLine(line_x, item->GetY(), line_x, item->GetY() + total_h);
		}
